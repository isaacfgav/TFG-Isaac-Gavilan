...
def resolver_columna_modelo(columna, columnas_modelo):
    """
    Devuelve el nombre real de la columna en la matriz del modelo.
    Permite casar nombres originales y nombres saneados tipo R make.names().
    """
    if columna in columnas_modelo:
        return columna

    columna_make = make_names_py(columna)
    if columna_make in columnas_modelo:
        return columna_make
...
def asegurar_lista(valor):
    """
    Normaliza valores procedentes de JSON.

    jsonlite::toJSON(auto_unbox=TRUE) convierte listas de longitud 1
    en strings simples. La app necesita tratar ambos casos igual:
    "edad_anos" -> ["edad_anos"].
    """
    if valor is None:
        return []
    ...
    if isinstance(valor, (list, tuple, set, np.ndarray, pd.Series)):
        salida = []
        for elemento in list(valor):
            salida.extend(asegurar_lista(elemento))
        return salida

    return [valor]


def resolver_columnas_modelo(columnas, columnas_modelo):
    resueltas = []
    for columna in asegurar_lista(columnas):
        resuelta = resolver_columna_modelo(str(columna), columnas_modelo)
        if resuelta is not None:
            resueltas.append(resuelta)
    return list(dict.fromkeys(resueltas))


def diagnostico_matriz_modelo(df_input, artifacts):
    """Devuelve un resumen compacto de la matriz que recibe XGBoost."""
    x_new = construir_matriz_modelo(df_input, artifacts)
    nonzero_por_fila = (x_new != 0).sum(axis=1)
    columnas_con_senal = x_new.columns[(x_new != 0).any(axis=0)].tolist()

    return {
        "n_filas": int(x_new.shape[0]),
        "n_columnas": int(x_new.shape[1]),
        "nonzero_min": int(nonzero_por_fila.min()) if len(nonzero_por_fila) else 0,
        "nonzero_max": int(nonzero_por_fila.max()) if len(nonzero_por_fila) else 0,
        "columnas_con_senal": columnas_con_senal,
        "columnas_sin_senal": [c for c in x_new.columns if c not in columnas_con_senal],
        "preview": x_new.head(5)
    }
...
def valor_defecto_variable(variable, artifacts, df_input=None):
    """
    Devuelve un valor seguro cuando falta una variable.
    Caso especial: la variable larga de fatiga no está en reference_train.csv,
    pero puede reconstruirse desde perc_fatiga.
    """
    mapa = artifacts.get("feature_map", {}).get(variable, {})

    if (
        "percepcion_de_fatiga" in str(variable)
        and df_input is not None
        and "perc_fatiga" in df_input.columns
    ):
        serie = _to_numeric_series(df_input["perc_fatiga"]).fillna(5)
        return serie.round().clip(1, 10).astype(int).astype(str)

    if mapa.get("type") == "categorical":
        niveles = [str(x) for x in mapa.get("levels", [])]
        valor = niveles[0] if niveles else ""
        if df_input is not None:
            return pd.Series([valor] * len(df_input), index=df_input.index)
        return valor

    meta_vars = artifacts.get("variables", [])
    meta = next((m for m in meta_vars if m.get("name") == variable), {})
    valor = valor_numerico_meta(meta.get("median"), 0.0)
    if df_input is not None:
        return pd.Series([valor] * len(df_input), index=df_input.index)
    return valor


def preparar_input_modelo(df_input, artifacts, allow_missing=False):
    df_preparado = df_input.copy()
    variables_originals = artifacts["variable_names"]

    variables_absents = [
        variable
        for variable in variables_originals
        if variable not in df_preparado.columns
    ]

    if variables_absents and not allow_missing:
        raise ValueError(
            "Falten variables d'entrada: "
            + ", ".join(variables_absents)
        )

    for variable in variables_absents:
        df_preparado[variable] = valor_defecto_variable(
            variable,
            artifacts,
            df_preparado
        )
...
def construir_matriz_modelo(df_input, artifacts, allow_missing=False):
    df_input = preparar_input_modelo(
        df_input,
        artifacts,
    ...
    for variable in variables_originals:
        ...
        if mapa.get("type") == "categorical":
            ...
            for nivell, columnes in nivell_columnes.items():
                ...
                columnes_valides = resolver_columnas_modelo(
                    columnes,
                    set(x_new.columns)
                )
            ...
            columnes = resolver_columnas_modelo(
                mapa.get("columns", []),
                set(x_new.columns)
            )
            ...
            if not columnes:
                ...
                columna_alternativa = resolver_columna_modelo(
                    alternativa,
                    set(x_new.columns)
                )
                ...
                columnes = (
                    [columna_alternativa]
                    if columna_alternativa is not None
                    else []
                )
...
def extraer_indices_prediccion(prediccio_bruta, n_filas, num_classes):
    prediccio_array = np.asarray(prediccio_bruta)

    if prediccio_array.ndim == 2:
        probabilitats_array = prediccio_array
        prediccio_numerica = np.argmax(probabilitats_array, axis=1)

    elif prediccio_array.size == n_filas * num_classes:
        probabilitats_array = prediccio_array.reshape((n_filas, num_classes))
        prediccio_numerica = np.argmax(probabilitats_array, axis=1)

    elif prediccio_array.size == n_filas:
        prediccio_numerica = np.rint(prediccio_array).astype(int)
        if prediccio_numerica.min() >= 1 and prediccio_numerica.max() <= num_classes:
            prediccio_numerica = prediccio_numerica - 1
        probabilitats_array = None

    else:
        raise ValueError(
            "Forma inesperada de la predicción XGBoost: "
            f"{prediccio_array.shape}. Filas={n_filas}, clases={num_classes}."
        )

    prediccio_numerica = np.clip(prediccio_numerica, 0, num_classes - 1)
    return prediccio_numerica, probabilitats_array


@st.cache_data(show_spinner=False)
def calcular_mapa_clases_desde_reference(_artifact_dir, artifacts, reference_train):
    """
    XGBoost devuelve clases numéricas 0,1,2. En este RDS el orden real de esas
    clases no coincide necesariamente con artifacts['nivells_k3'].
    Esta función calibra el mapeo usando reference_train.csv y su columna k3.
    """
    nivells = [str(x) for x in artifacts["nivells_k3"]]
    num_classes = len(nivells)

    if reference_train.empty or "k3" not in reference_train.columns:
        return {str(i): nivells[i] for i in range(num_classes)}

    booster_tmp = xgb.Booster()
    booster_tmp.load_model(str(Path(_artifact_dir) / "xgboost_model.json"))

    x_ref = construir_matriz_modelo(reference_train, artifacts, allow_missing=True)
    dref = xgb.DMatrix(
        x_ref.to_numpy(dtype=float),
        feature_names=list(x_ref.columns)
    )

    pred_raw = booster_tmp.predict(dref)
    pred_idx, _ = extraer_indices_prediccion(
        pred_raw,
        len(reference_train),
        num_classes
    )

    reales = reference_train["k3"].astype(str).to_numpy()
    mejor_score = -1
    mejor_perm = nivells

    for perm in itertools.permutations(nivells):
        pred_labels = np.array([perm[i] for i in pred_idx])
        score = int((pred_labels == reales).sum())
        if score > mejor_score:
            mejor_score = score
            mejor_perm = perm

    return {str(i): str(mejor_perm[i]) for i in range(num_classes)}
...
def predir_amb_model(df_input, booster, artifacts):
    ...
    prediccio_numerica, probabilitats_array = extraer_indices_prediccion(
        prediccio_bruta,
        len(df_input),
        num_classes
    )

    mapa_clases = globals().get(
        "MAPA_CLASES_XGBOOST",
        {str(i): nivells[i] for i in range(num_classes)}
    )

    etiquetas_por_indice = [
        str(mapa_clases.get(str(i), nivells[i]))
        for i in range(num_classes)
    ]

    probabilitats = None
    if probabilitats_array is not None:
        probabilitats = pd.DataFrame(
            probabilitats_array,
            columns=[f"prob_{etiquetas_por_indice[i]}" for i in range(num_classes)],
            index=df_input.index
        )

    clusters = [etiquetas_por_indice[int(index)] for index in prediccio_numerica]
...
MAPA_CLASES_XGBOOST = calcular_mapa_clases_desde_reference(
    str(ARTIFACT_DIR),
    artifacts,
    reference_train
)
...
with st.sidebar:
    st.caption(
        "Mapa clases XGBoost: "
        + ", ".join([f"{k}→{v}" for k, v in MAPA_CLASES_XGBOOST.items()])
    )
...
with tab_enquesta:
    ...
    if enviar:
        ...
        try:
            ...
            with st.expander("🔎 Diagnòstic de la matriu enviada a XGBoost"):
                diag = diagnostico_matriz_modelo(
                    df_enquesta,
                    artifacts
                )
                st.write(
                    f"Columnes model: {diag['n_columnas']} · "
                    f"Variables no-zero: {diag['nonzero_min']}"
                )
                st.dataframe(
                    diag["preview"],
        ...
        try:
            ...
            if st.button(
                ...
                with st.expander("🔎 Diagnòstic de la matriu enviada a XGBoost"):
                    ...
                    st.write(
                        f"Files: {diag['n_filas']} · Columnes model: {diag['n_columnas']} · "
                        f"Variables no zero per fila: {diag['nonzero_min']} - {diag['nonzero_max']}"
                    )
                    st.caption(
                        "Si aquí surt 0 o molt poques variables no-zero, "
                        "el model no està rebent correctament la informació."
                    )
                    st.dataframe(
                        diag["preview"],
                        use_container_width=True
