        ...
        for idx, var_meta in enumerate(variables_formulari):
            ...
            with cols[idx % 2]:
                if es_variable_fatiga_slider(nom):
                    min_v = float(var_meta.get("min") if var_meta.get("min") is not None else 0)
                    max_v = float(var_meta.get("max") if var_meta.get("max") is not None else 10)
                    med = float(var_meta.get("median") if var_meta.get("median") is not None else 5)
                    ...
                    kwargs = {"label": etiqueta, "value": float(var_meta.get("median") or 0), "help": f"Variable original: {nom}"}
                    if var_meta.get("min") is not None: kwargs["min_value"] = float(var_meta.get("min"))
                    if var_meta.get("max") is not None: kwargs["max_value"] = float(var_meta.get("max"))
                    respostes[nom] = st.number_input(**kwargs)

        st.markdown("### Lesions i afectacions")
        c1, c2 = st.columns(2)
        with c1:
            lesions_sel = st.multiselect("Selecciona les localitzacions de lesió", [v.get("name") for v in variables_lesions], format_func=netejar_nom_variable)
        with c2:
            afectacions_sel = st.multiselect("Selecciona les estructures afectades", [v.get("name") for v in variables_afectacions], format_func=netejar_nom_variable)

        for vm in variables_lesions:
            respostes[vm.get("name")] = valor_binari_model(vm, vm.get("name") in lesiones_sel)
