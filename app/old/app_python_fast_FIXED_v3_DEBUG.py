...
def predir_amb_model(df_input, booster, artifacts):
    ...
    # - multi:softprob -> vector plano n * clases o matriz n x clases
    # - multi:softmax -> vector n con índices de clase
    ...
    if prediccio_array.ndim == 2:
    ...
    elif prediccio_array.size == len(df_input) * num_classes:
        probabilitats_array = prediccio_array.reshape(
            (len(df_input), num_classes)
        )
        prediccio_numerica = np.argmax(probabilitats_array, axis=1)

    elif prediccio_array.size == len(df_input):
        prediccio_numerica = np.rint(prediccio_array).astype(int)

        # Por seguridad: si el modelo devuelve 1,2,3 en lugar de 0,1,2,
        # lo pasamos a índice Python 0,1,2.
        ...
        raise ValueError(
            ...
            f"{prediccio_array.shape}. Filas={len(df_input)}, clases={num_classes}."
        )

    prediccio_numerica = np.clip(
        prediccio_numerica,
        0,
        num_classes - 1
    )
    ...
    if probabilitats_array is not None:
        probabilitats = pd.DataFrame(
            probabilitats_array,
            columns=[f"prob_{classe}" for classe in nivells],
            index=df_input.index
        )

    clusters = [nivells[index] for index in prediccio_numerica]
