...
if (is.null(model) || is.null(dummy_model) || is.null(nivells_k3) || is.null(variables_train)) {
  stop("El fitxer RDS no té l'estructura esperada. Ha de contenir model, dummy_model, nivells_k3 i variables_train.")
}
...
# ------------------------------------------------------------------------------
# Variables originals que espera el model
...
# ------------------------------------------------------------------------------
# Convertim les dades al format dummy utilitzat en l'entrenament
...
cat("Predicció completada correctament.\n")